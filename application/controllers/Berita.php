...
class Berita extends CI_Controller
{
	...
	public function index()
	{
		...
		$populer	= $this->berita_model->populer_new();

		// Berita dan paginasi
		// Berita dan paginasi
		$this->load->library('pagination');
		$config['base_url'] 		= base_url() . 'berita/index/';
		$config['total_rows'] 		= count(array($this->berita_model->total_new()));
	...
	public function kategori($slug_kategori)
	{
		...
		$berita 	= $this->berita_model->kategori_new($kategori_name, $config['per_page'], $page);
	...
	{
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->reader($slug_berita);
		$listing 	= $this->berita_model->listing_reader();
		$kategori 	= $this->nav_model->nav_berita();
		$berita_detail = $this->berita_model->reader_detail($berita->kode_berita);
		$data = array(
			'title'		=> $berita->judul,
			'deskripsi'	=> $berita->detail1,
			'keywords'	=> $berita->judul,
			'berita'	=> $berita,
			'detail_berita' => $berita_detail,
			'listing'	=> $listing,
			'kategori'	=> $kategori,
			'site'		=> $site,
			'isi'		=> 'berita/read3'
